...
import me.ixor.sred.SRED
import me.ixor.sred.WorkflowResult
import me.ixor.sred.core.logger
import me.ixor.sred.registration.RegistrationContext
import me.ixor.sred.registration.RegistrationFixtures
import me.ixor.sred.registration.RegistrationService
import me.ixor.sred.registration.RegistrationStates
...
private suspend fun executeWorkflow(
    ...
    return SRED.fromConfig(
        ...
        if (emailSent) {
            delay(300)
            val code = context.verificationCodeRepository.findCode(email)
            code?.let {
        ...
        if (currentState == RegistrationStates.WAITING_VERIFICATION) {
            delay(500) // 模拟用户等待
            val code = context.verificationCodeRepository.findCode(email) ?: verificationCode
            if (!emailSent) {
