...
import me.ixor.sred.transfer.TransferFixtures
import me.ixor.sred.transfer.TransferService
...
fun runMultiWorkflow(args: Array<String>) {
    runBlocking {
        ...
        // 1. 创建引擎（使用转账流程作为默认流程）
        val transferContext = TransferFixtures.createDefault()
        val engine = SRED.fromConfig(
            configPath = "transfer.json",
            dbPath = "multi_workflow.db",
            handlers = TransferService(
                transferContext.userRepository,
                transferContext.accountRepository
            )
        )
        ...
        engine.addHandler(
            RegistrationService(
                registrationContext.userRepository,
                registrationContext.verificationCodeRepository
            )
        )
