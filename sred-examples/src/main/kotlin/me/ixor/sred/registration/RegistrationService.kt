 ...
 * 用户注册服务 - 状态处理器
 *
 * 与 UI、示例入口解耦，只依赖抽象的 StateContext。
 */
@Service
class RegistrationService(
    private val userRepository: RegistrationUserRepository,
    ...
    suspend fun validateUser(context: StateContext): StateResult {
        ...
        return when {
            ...
            userRepository.existsByUsername(username) -> StateResult.failure("用户名已存在")
            else -> {
                userRepository.save(RegistrationUser(username, email, password))
                StateResult.success(
                    mapOf(
                        "validated" to true
                    )
    ...
    suspend fun verifyCode(context: StateContext): StateResult {
        ...
        val expectedCode = verificationCodeRepository.findCode(email)
        // 调试日志：对比期望与输入
